...
#define SNAPLEN 65535  // Increased from BUFSIZ to capture full packets
...
void finalize_alerts(AlertContext *context) {
    if (json_array_size(context->alerts) > 0) {
        ...
        if (fp) {
            ...
            snprintf(scp_command, sizeof(scp_command), "scp %s robot@192.168.100.24:/home/robot/edr_server/alerts/", context->alert_filename);
            system(scp_command);
        }
    }
...
void packet_handler(u_char *user_data, const struct pcap_pkthdr *pkthdr, const u_char *packet) {
    ...
    // Always write the packet to the pcap file first
    ...
    if (ip_header->ip_p == IPPROTO_TCP && 
        ...
        // Existing detection logic for TCP-based attacks
        if (dst_port == 22) {
...
void capture_traffic(const char *interface) {
    ...
    // Open interface with improved settings
    handle = pcap_open_live(interface, 
                           SNAPLEN,  // Increased from BUFSIZ to capture full packets
                           1,        // Promiscuous mode
                           1000,     // Timeout (ms)
                           errbuf);
    if (handle == NULL) {
    ...
    // Use a broader filter to ensure we capture all necessary packets
    ...
    // Capture packets until we reach MAX_PACKETS or get a signal
    ...
    // Transfer the pcap file to the server
    char scp_command[512];
    snprintf(scp_command, sizeof(scp_command), "scp %s robot@192.168.100.24:/home/robot/edr_server/pcap_files/", current_pcap_filename);
    system(scp_command);
}
