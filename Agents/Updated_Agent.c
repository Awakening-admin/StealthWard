...
char* get_endpoint_ip() {
    ...
    if (ip != NULL && strlen(ip) > 0) return ip;
    ...
    if (fp) {
        ...
        if (json_array_size(context->alerts) > 0) {
            ...
            if (system(mkdir_command) != 0) {
                console_log("Warning: Failed to create remote alerts directory");
            }
            ...
            if (system(scp_command) != 0) {
                console_log("Error: Failed to transfer alerts file");
            }
        }
    }
    free(endpoint_ip);
...
void capture_traffic(const char *interface) {
    ...
    if (system(mkdir_command) != 0) {
        console_log("Warning: Failed to create remote pcap directory");
    }
    ...
    if (system(scp_command) != 0) {
        console_log("Error: Failed to transfer pcap file");
    }
